...
public class MainPaneFX2 {
	 ...
	 */
	MainPaneFX2(double CANVAS_WIDTH, double CANVAS_HEIGHT) {
	    	...
	    	graphCanvas.setVisible(false);
			 ...
			 if(choice != 0) {
				 ...
				 try {
					 ...
					 //JOptionPane.showMessageDialog(null, optimizerManager.answerString(choice,opt.getOptimum(),opt.getOptX(),opt.getOptY(),opt.getOptZ()));
					 new Alert(Alert.AlertType.INFORMATION, optimizerManager.answerString(choice,opt.getOptimum(),opt.getOptX(),opt.getOptY(),opt.getOptZ())).showAndWait();
		...
		do {
			try {
				...
				if (returnVal<1 || returnVal>5) {
					...
					new Alert(Alert.AlertType.ERROR, "Choice of function must be an integer between 1 and 3").showAndWait();
					}
				else error = false;
			}
			catch (NumberFormatException e) {
				//JOptionPane.showMessageDialog(null, "Input Error: "+e);
				new Alert(Alert.AlertType.ERROR, "Input error!"+e).showAndWait();
				error = true;
			}
		} while(error);
		return returnVal;
